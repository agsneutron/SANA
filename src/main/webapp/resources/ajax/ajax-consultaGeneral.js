...
function verDatos() {
    ...
    if (fechaInicio==""){fechaInicio = f.getFullYear() + "-" + (f.getMonth() +1) + "-" + f.getDate() ;}
    if (fechaFin==""){fechaFin = f.getFullYear() + "-" + (f.getMonth() +1) + "-" + f.getDate();}
...
function despliegaTabla(Datos){
	...
	var stabla = '<table id="tablaGeneral" class="table table-bordered order-column">'
    +'<thead>'
    +'  <tr>'
    +'    <th>Visitados</th>'
    +'    <th>Por Visitar</th>'
	...
	+ '<tr>'
			+'<th scope="row">' + objJson.visitados + '</th>'
			+'<td>' + objJson.porVisitar + '</td>'
			+'<td>' + objJson.cobranzaTotal + '</td>'
			+'<td>' + objJson.porCobrar + '</td>'
